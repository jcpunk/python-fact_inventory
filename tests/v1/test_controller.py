...
class TestFactControllerSubmit:
    ...
    async def test_submit_missing_system_facts(
        ...
        response = await client.post(
            "/v1/facts",
            json={"package_facts": {}, "local_facts": {}},
        )
    ...
    async def test_submit_missing_package_facts(
        ...
        response = await client.post(
            "/v1/facts",
            json={"system_facts": {}, "local_facts": {}},
        )

        assert response.status_code == HTTP_400_BAD_REQUEST

    async def test_submit_missing_local_facts(
        self,
        client: AsyncTestClient,
    ) -> None:
        """Test that missing local_facts field is rejected."""
        response = await client.post(
            "/v1/facts",
    ...
    async def test_submit_non_dict_local_facts(
        self,
        client: AsyncTestClient,
    ) -> None:
        """Test that non-dict local_facts are rejected."""
        response = await client.post(
            "/v1/facts",
            json={
                "system_facts": {},
                "package_facts": {},
                "local_facts": ["not", "a", "dict"],
            },
        )
    ...
    async def test_submit_empty_string_value_in_facts(
        ...
        response = await client.post(
            "/v1/facts",
            json={"system_facts": {"key": ""}, "package_facts": {}, "local_facts": {}},
        )
        assert response.status_code == HTTP_201_CREATED
