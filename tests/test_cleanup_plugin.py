...
class TestDailyCleanupPluginLifecycle:
    ...
    async def test_completion_log_includes_count_and_duration(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        """Completion log must include records_purged and duration_seconds."""

        async def _purge_some() -> int:
            return 42

        plugin = DailyCleanupPlugin(
            cleanup_fn=_purge_some,
            interval_seconds=_MIN_INTERVAL_SECONDS,
            name="test-log",
        )
        plugin._interval = 0
        ...
        completion_lines = [
            r.message for r in caplog.records if "completed" in r.message
        ]
        assert completion_lines, "No completion log line found"
        line = completion_lines[0]
        assert "records_purged=42" in line
