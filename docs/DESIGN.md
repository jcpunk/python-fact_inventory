...
- `ix_fact_inventory_system_facts`: GIN index for efficient JSONB queries (PostgreSQL only; no-op on other backends).
- `ix_fact_inventory_package_facts`: GIN index for efficient JSONB queries (PostgreSQL only; no-op on other backends).
