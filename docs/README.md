...
- **Automatic Data Retention**: Background task periodically purges records older than `RETENTION_DAYS` (default: 400 days)
- **PostgreSQL Optimized**: Uses `JSONB` fields with `GIN` indexes for efficient querying (fallback support for SQLite)
...
| `RETENTION_DAYS`          | no       | `400`            | Days to retain host records before automatic purge                                                                                                                                                                         |
...
| `MAX_JSON_FIELD_MB`       | no       | `4`              | Maximum size in MB for a single JSON field (`system_facts`, `package_facts`, `local_facts`). Validated before each record is written to the database                                                                       |
| `MAX_REQUEST_BODY_MB`     | no       | `13`             | Maximum total request body size in MB, enforced at the HTTP layer. Must be greater than `3 x MAX_JSON_FIELD_MB` to leave room for both JSON fields and the surrounding request envelope                                    |
...
`RETENTION_DAYS` (default 400). The cleanup runs every
...
- Increase `MAX_JSON_FIELD_MB` to allow larger per-field JSON payloads
- Increase `MAX_REQUEST_BODY_MB` to allow a larger total request body
- `MAX_REQUEST_BODY_MB` must always be greater than `3 x MAX_JSON_FIELD_MB`
