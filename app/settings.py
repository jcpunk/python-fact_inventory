...
Configurable elements (production):
  ...
  - RETENTION_DAYS: int            (default 400)
  ...
  - MAX_REQUEST_BODY_MB: int       (default 13; max total request body size
                                    in MB enforced at the HTTP layer; must
                                    be greater than 3 x MAX_JSON_FIELD_MB
...
class Settings(BaseSettings):
    ...
    def _check_body_size(self) -> Self:
        ...
        overhead.  Requiring max_request_body_mb > 3 * max_json_field_mb
        preserves this invariant regardless of the chosen field size.
        """
        if self.max_request_body_mb <= 3 * self.max_json_field_mb:
            ...
            raise ValueError(  # noqa: TRY003
                ...
                f" than 3 x max_json_field_mb ({3 * self.max_json_field_mb})"
            )
        return self
