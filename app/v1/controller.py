...
_SUBMIT_SUCCESS_RESPONSES: dict[int, Any] = {
...
class FactInventoryController(Controller):
    ...
    async def submit(
        ...
        Request validation is delegated to the DTO layer
        (:class:`FactInventoryWriteAPI`).  Rate limiting is enforced
        by the middleware before this handler runs.  Persistence is
        delegated to the injected service.
        """
        ...
        logger.info(
            "Stored facts for client %s - fact count system:%d package:%d local:%d",
            client_address,
            len(data.system_facts),
            len(data.package_facts),
            len(data.local_facts),
        )
        return Response(
