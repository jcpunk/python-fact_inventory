...
def create_app() -> Litestar:
    ...
    Wires together the database plugin, background cleanup task,
    observability middleware (OpenTelemetry always-on, Prometheus
    optional), rate-limited route handlers, and logging.  All tunables
    are read from the :data:`settings` singleton.
    ...
    -------
    Litestar
        Ready to hand to an ASGI server such as Uvicorn.
    """
    ...
    async def _purge_expired_facts() -> int:
        ...
        Opens a short-lived session that is independent of any
        request-scoped session, delegates to
        :meth:`~app.v1.services.FactInventoryService.purge_expired_facts`,
        and returns the deleted-row count so the cleanup plugin can
        include it in its completion log line.
        """
        async with alchemy_config.get_session() as session:
            service = FactInventoryService(session)
            return await service.purge_expired_facts(settings.retention_days)
